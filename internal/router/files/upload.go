...
func Upload(conf *config.Config, store *event.Store) func(*gin.Context) {
	return func(ctx *gin.Context) {
		...
		tmpDir := filepath.Join(os.TempDir(), conf.UploadTemp, username.(string))
		if err := os.MkdirAll(tmpDir, 0755); err != nil {
			ctx.String(http.StatusInternalServerError, fmt.Sprintf("failed to create temp dir: %v", err))
			return
		}

		finalDir := filepath.Join(conf.UploadFinal, username.(string), "tmp")
		...
		for fieldName, fileHeader := range form.File {
			...
			if err != nil {
				...
				log.Printf("failed to parse fieldParts: %v\n", err)
				continue
			}
			...
			if err := saveChunk(ctx, fileHeader, chunkPath); err != nil {
				log.Printf("[%s] failed to save chunks: %v\n", fieldParts.filename, err)
				continue
			}

			// 3. validate chunk (md5)
			if err := validateChunk(chunkPath, fieldParts.md5hash); err != nil {
				log.Printf("[%s] failed to validate chunks: %v\n", fieldParts.filename, err)
				os.Remove(chunkPath)
				continue
			}

			// 4. move tmp to final path
			finalPath := filepath.Join(conf.UploadFinal, username.(string), "tmp", fieldParts.filename, fieldParts.chunkIdxStr)

			if err := finalizeChunk(chunkPath, finalPath); err != nil {
				log.Printf("[%s] failed to finalize chunks: %v\n", fieldParts.filename, err)
				continue
			}
		...
		store.Register(event.FileUploaded(username.(string), finalDir, event.WithResultChan(rchan)))
